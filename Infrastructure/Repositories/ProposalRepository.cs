    ...
    public class ProposalRepository : BaseRepository<Proposal, int>, IProposalRepository
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var userInscriptionModalities = await _dbContext.Set<UserInscriptionModality>()
                ...
                .Include(uim => uim.User)
                .AsNoTracking()
                .ToListAsync(cancellationToken);            // 4. Agrupar por propuesta y construir resultado
            var usersByModality = userInscriptionModalities
                .GroupBy(uim => uim.IdInscriptionModality)
                .ToDictionary(g => g.Key, g => g.ToList());
        ...
        public async Task<ProposalWithDetails?> GetProposalWithDetailsAsync(
            ...
            // 2. Cargar todos los UserInscriptionModalities y usuarios en una sola consulta
            var userInscriptionModalities = await _dbContext.Set<UserInscriptionModality>()
                .Where(uim => uim.IdInscriptionModality == id)
                .Include(uim => uim.User)
                .AsNoTracking()
                .ToListAsync(cancellationToken);

            // 3. Construir el resultado
            return new ProposalWithDetails
            {
                Proposal = proposal,
        ...
        public async Task<PaginatedResult<ProposalWithDetails>> GetAllProposalsWithDetailsPaginatedAsync(
            int pageNumber,
            int pageSize,
            string sortBy,
            bool isDescending,
