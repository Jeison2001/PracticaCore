...
{
    public class CoTerminalRepository : BaseRepository<CoTerminal, int>, ICoTerminalRepository
    {
        public CoTerminalRepository(AppDbContext context) : base(context)
        {
        }

        public async Task<CoTerminalWithDetails?> GetWithDetailsAsync(int id)
        {
            var entity = await _context.CoTerminals
        ...
        public async Task<PaginatedResult<CoTerminalWithDetails>> GetAllWithDetailsPaginatedAsync(
            int pageNumber, 
            int pageSize, 
            string sortBy, 
            bool isDescending, 
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var query = _context.CoTerminals
                .Include(x => x.InscriptionModality)
                    .ThenInclude(im => im.Modality)
                .Include(x => x.InscriptionModality)
                    .ThenInclude(im => im.StateInscription)
            ...
            query = query.ApplyFilters<CoTerminal, int>(filters);
            ...
            query = (sortBy?.ToLower() ?? "default") switch
            {
                ...
                "universityname" => isDescending ? query.OrderByDescending(x => x.UniversityName) : query.OrderBy(x => x.UniversityName),
                "programname" => isDescending ? query.OrderByDescending(x => x.PostgraduateProgramName) : query.OrderBy(x => x.PostgraduateProgramName),
            ...
            var resultItems = new List<CoTerminalWithDetails>();
            foreach (var item in items)
            {
                resultItems.Add(await PopulateDetails(item, cancellationToken));
            }

            return new PaginatedResult<CoTerminalWithDetails>
            {
                Items = resultItems,
        ...
        public async Task<List<CoTerminalWithDetails>> GetByUserAsync(int userId, bool? status = null, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<PaginatedResult<CoTerminalWithDetails>> GetByTeacherAsync(
            int teacherId,
            int pageNumber, 
            int pageSize, 
            string sortBy, 
        ...
        private async Task<CoTerminalWithDetails> PopulateDetails(CoTerminal entity, CancellationToken cancellationToken = default)
        {
            ...
            return new CoTerminalWithDetails
            {
                CoTerminal = entity,
        ...
        private IQueryable<CoTerminal> ApplySpecificFilters(IQueryable<CoTerminal> query, Dictionary<string, string> filters)
        {
